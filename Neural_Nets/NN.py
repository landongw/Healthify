...
def generate_Model(optimizer='adam', loss='categorical_crossentropy', activation=tf.nn.relu):
    ...
    model.add(tf.keras.layers.Dense(1024, activation=activation))  # a simple fully-connected layer, 1024 units, relu activation
    # model.add(tf.keras.layers.Dense(512, activation=activation))  # a simple fully-connected layer, 512 units, relu activation
    model.add(tf.keras.layers.Dense(256, activation=activation))  # a simple fully-connected layer, 256 units, relu activation
